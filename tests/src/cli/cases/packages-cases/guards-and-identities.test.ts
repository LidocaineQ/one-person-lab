...
test('OPL Flow manifest resolves its package-owned 0.1.28 carrier and managed policy payload', () => {
  ...
  assert.deepEqual(manifest.required_skill_ids, ['coordinate-concurrent-tasks', 'opl-flow']);
  assert.equal(
    manifest.plugin_payload_manifest_url,
    path.join(repoRoot, 'contracts', 'opl-framework', 'packages', 'payloads', 'opl-flow-0.1.28.json'),
  );
