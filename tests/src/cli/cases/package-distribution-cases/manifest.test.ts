...
test('packages manifest exposes canonical Release Set coordinates for Package install updates', () => {
  ...
  assert.equal(output.packages_manifest.packages.package_artifacts['opl-flow'].package_version, '0.1.30');
  ...
  assert.equal(
    ...
    'ghcr.io/gaofeng21cn/one-person-lab-packages/opl-flow:0.1.30',
  );
  assert.equal(
