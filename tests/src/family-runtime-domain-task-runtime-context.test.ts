...
test('dead-letter attempt query remains readable after the linked task row is deleted', () => {
  const db = new DatabaseSync(':memory:');
  try {
    createFamilyRuntimeQueueTables(db);
    const taskId = 'frt-domain-runtime-context-dead-letter';
    const payload = currentTaskPayload();
    insertTask(db, { taskId, payload });
    const attempt = createAttempt(db, taskId, payload);
    db.prepare(`
    ...
    assert.equal(query.temporal_durable_lifecycle_readback?.workflow_history_identity.workflow_id, attempt.workflow_id);
  } finally {
    db.close();
  }
});
