...
} from './cli/cases/domain-pack-compiler-fixtures.ts';
import {
...
test('generic substrate projection ignores retired private domain-handler export controls', () => {
  ...
  try {
    ...
    const inspect = runCli(['substrate', 'projection', '--domain', 'medautoscience'], {
      OPL_CONTRACTS_DIR: fixtureContractsRoot,
      OPL_STATE_DIR: stateRoot,
      OPL_FAMILY_RUNTIME_MEDAUTOSCIENCE_EXPORT: '/retired/private/domain-handler-export',
    });
    const projection = inspect.generic_substrate_projection;

    assert.equal(projection.projection_status, 'substrate_refs_partial');
    assert.equal('domain_handler_substrate_adapter' in projection, false);
    assert.equal(projection.workspace_refs.status, 'resolved_from_manifest');
    assert.deepEqual(projection.workspace_refs.refs, []);
    assert.equal(projection.source_refs.refs.some((entry: { ref_id: string }) =>
      entry.ref_id === 'source_provenance_ref'
    ), true);
    assert.equal(projection.artifact_refs.refs.some((entry: { ref_id: string }) =>
      entry.ref_id === 'publication_eval_latest'
    ), true);
    assert.equal(projection.memory_refs.status, 'missing');
    assert.equal(projection.non_authority_flags.opl_reads_memory_body, false);
    assert.equal(projection.non_authority_flags.opl_writes_domain_truth, false);
    assert.equal(projection.non_authority_flags.opl_authorizes_quality_verdict, false);
  } finally {
...
test('generic substrate workbench groups family refs for App and operator drilldown without body authority', () => {
  ...
  const fixtures = loadFamilyManifestFixtures();
  const workspaceRoot = createFamilyDefaultContractWorkspace();
  const masManifest = withMasLikeMemoryDescriptor(withPackCompilerReadySurfaces(
    fixtures.medautoscience,
    {
      agentId: 'mas',
  ...
  const magManifest = withPackCompilerReadySurfaces(fixtures.medautogrant, {
    agentId: 'mag',
  ...
  const rcaManifest = withPackCompilerReadySurfaces(fixtures.redcube, {
    agentId: 'rca',
  ...
  try {
    [
      ...
      ['medautogrant', 'med-autogrant', magManifest],
      ['redcube', 'redcube-ai', rcaManifest],
    ...
    assert.equal('domain_handler_adapter_resolved_count' in workbench.summary, false);
    assert.equal('domain_handler_substrate_adapter_status' in workbench.groups.by_domain.medautoscience, false);
    assert.equal(workbench.groups.by_domain.medautogrant.inspect_command, 'opl substrate projection --domain medautogrant');
    assert.equal(workbench.groups.by_domain.redcube.status_by_ref_family.artifact, 'missing');
    assert.ok(workbench.groups.by_projection_status.substrate_refs_resolved.includes('medautoscience'));
    assert.ok(workbench.groups.by_projection_status.substrate_refs_partial.includes('medautogrant'));
    assert.ok(workbench.groups.by_projection_status.substrate_refs_partial.includes('redcube'));
    assert.equal('by_domain_handler_status' in workbench.groups, false);
    ...
    assert.ok(workbench.groups.by_ref_family.source.some(
      (entry: { project_id: string; ref_id: string }) =>
        entry.project_id === 'medautoscience' && entry.ref_id === 'source_provenance_ref',
    ));
    assert.ok(workbench.groups.by_ref_family.artifact.some(
      (entry: { project_id: string; ref_id: string }) =>
        entry.project_id === 'medautogrant' && Boolean(entry.ref_id),
    ));
    assert.equal(workbench.groups.by_ref_family.artifact.some(
