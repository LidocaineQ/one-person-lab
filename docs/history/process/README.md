...
| Retired dated boundary and capability audits | The 2026-07-07 domain-boundary/capability-admission audits and the 2026-07-08 standard-agent adjustment no longer remain in current support. Their fixed counts, per-family cleanup boards, implementation matrices and proof lists are recoverable from git history, not current backlog. Do not recreate those files as active support; durable rules stay in status, policy, registry source and fresh readback. | `docs/status.md`, `docs/policies/standard-agent-capability-management-policy.md`, `src/kernel/standard-agent-registry.ts`, `opl agents conformance|default-callers --family-defaults --json`, git history |
