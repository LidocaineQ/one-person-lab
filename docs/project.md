...
`OPL Framework` 的内部能力当前仍以十个品牌模块作为认知与源码导航：`OPL Charter`、`OPL Atlas`、`OPL Workspace`、`OPL Pack`、`OPL Stagecraft`、`OPL Runway`、`OPL Ledger`、`OPL Console`、`OPL Foundry Kernel` 和 `OPL Connect`。这组模块不替代 `OPL Base + OPL App + OPL Packages + OPL Cloud` 的产品面，也不等于十个 Package 或十个 Cordis plugin。当前用户工作面是 App desktop + Docker/WebUI；Cloud 的具体能力以真实 account、storage、isolation、backend 与 owner policy 证据为准。品牌模块、authority owner、Package 发布单元和 Cordis plugin contribution 按真实 caller 与生命周期分别调整，不为目录对称而绑定。
