...
源码物理组织正在从品牌桶 successor-first 重排为 `src/authority/**`、`src/adapters/**`、`src/read-models/**`、`src/host/**` 和薄 `src/entrypoints/**`。`src/modules/<module_id>/` 仅在 cutover 期间作为 compatibility source；必须先建立新 authority/adapter/read-model/Host successor、切真实 caller、验证 affected outcome与 public export，再以 structural caller=0退役旧路径。`contracts/opl-framework/source-module-map.json` 在迁移期负责归属/历史 readback，不替代最终 topology，也不是 Cordis registry。
