...
当前 repo 分工固定为：`one-person-lab` 是 OPL Framework / runtime / CLI / contracts owner，并为 `OPL Base` 的 headless / 独立安装维护自包含 Codex carrier，但不持有 GUI shell implementation；`one-person-lab-app` 是 GUI product truth、release、updater、用户教程、页面状态、active-shell validation 和 GUI candidate policy owner；`opl-aion-shell` 是当前 OPL-branded AionUI shell implementation carrier，负责实现 App-owned GUI contract，并从 bundled AionCore managed-resources 选择 App 内唯一 Codex executable。App Full runtime 不重复嵌入 Framework Codex carrier。`opl-studio` 是 App-owned DSH-derived foreground alternative GUI candidate，通过同一 `OPL_CODEX_BIN` / Codex App Server 边界使用 Studio-owned 或 exact external binary，不继承 AionCore dependency；Hermes Desktop / `hermes-codex` 是 retained explicit reference candidate；`opl-agui-codex-shell` / `agui-codex` 只作为 AG-UI/CopilotKit archived technical proof 与显式 replay surface 保留。MAS/MAG/RCA/Book Forge 等 domain repos 是 domain app/runtime authority，持有自己的 stage 语义、prompt、skill、领域逻辑、质量 gate、truth reducer、运行规则、交付物真相、owner receipt 和 direct skill 入口。
...
`OPL Framework` 的内部能力当前仍以十个品牌模块作为认知与源码导航：`OPL Charter`、`OPL Atlas`、`OPL Workspace`、`OPL Pack`、`OPL Stagecraft`、`OPL Runway`、`OPL Ledger`、`OPL Console`、`OPL Foundry Kernel` 和 `OPL Connect`。这组模块不替代 `OPL Base + OPL App + OPL Packages + optional OPL Cloud` 的产品面，也不等于十个 Package 或十个 Cordis plugin。当前用户工作面是 App desktop + Docker/WebUI；Cloud 只有在真实 account、storage、isolation、backend 与 owner policy 齐备时启用。品牌模块、authority owner、Package 发布单元和 Cordis plugin contribution 按真实 caller 与生命周期分别调整，不为目录对称而绑定。
