...
当前 repo 分工固定为：`one-person-lab` 是 OPL Framework Host / runtime / CLI / contracts owner，并为 `OPL Base` 的 headless / 独立安装维护自包含 Codex carrier，但不持有 GUI renderer/product truth；`one-person-lab-app` 是 App product/profile truth、Client Contribution ABI、release、updater、用户教程、页面状态、active-shell validation 和 GUI candidate policy owner；`opl-aion-shell` 是当前 OPL-branded AionUI renderer/carrier，DSH GUI是 App-owned candidate renderer，二者必须消费同一 Framework Host projected client graph 与 App profile，不各自创建 Client registry/currentness。App Full runtime 不重复嵌入 Framework Codex carrier。`opl-native-workbench` 是 App-owned foreground alternative GUI candidate，通过同一 `OPL_CODEX_BIN` / Codex App Server 边界使用 Native-owned 或 exact external binary，不继承 AionCore dependency；Hermes Desktop / `hermes-codex` 是 retained explicit reference candidate；`opl-agui-codex-shell` / `agui-codex` 只作为 AG-UI/CopilotKit archived technical proof 与显式 replay surface 保留。MAS/MAG/RCA/Book Forge 等 domain repos 是 domain app/runtime authority，持有自己的 stage 语义、prompt、skill、领域逻辑、质量 gate、truth reducer、运行规则、交付物真相、owner receipt 和 direct skill 入口。
...
源码物理组织已从品牌桶重排为责任导向的 13 个 source units，落在 6 个 target roots：`src/authority/**`、`src/adapters/**`、`src/read-models/**`、`src/host/**`、`src/entrypoints/**` 和 `src/kernel/**`。`src/modules/**` 是历史路径，当前分支已完成 successor-first caller cutover 并标记为 `retired`/`must_be_absent`；它不再是 compatibility source、public barrel 或当前 owner。`contracts/opl-framework/source-module-map.json` 是机器 source-unit/legacy-root contract，`contracts/opl-framework/package-topology.json` 是 Package/source/plugin 关联 contract；二者都不是 Cordis registry。当前分支的物理重排与验证仍是 candidate，需 canonical/main 吸收后才可作为 Framework 当前事实，且不等于 release、production、App 或 Cloud 完成。
...
2026-06-10 后，原十个 family capability domains 共同描述 Foundry Agent OS pattern：`OPL Agent OS + Domain Declarative Pack + Domain Minimal Authority Kernel + Domain Capability Registry`。Agent Tool Arsenal / Capability Invocation OS 不新增独立品牌域；它由 Pack、Atlas、Stagecraft、Console、Connect、Runway和 Ledger等 domain的真实 authority/contribution协作承接，而不是要求它们保持十个对称目录或 plugin。Capability Registry和 Tool Arsenal都不执行 capability，不生成 owner receipt、typed blocker或 domain verdict。
...
2. `One Person Lab App`
   普通用户使用的工作台产品。它消费 OPL Framework 的 Host/runtime/activation truth，把通用工作和 Foundry Agents 呈现为可直接使用的桌面体验；它持有 App product profile、Client Contribution ABI、page/action policy、release gate、updater metadata、用户文档和 active shell validation，不持有 domain truth，也不复制 runtime/provider 实现。AionUI主线和 DSH GUI候选从同一 Host-projected client graph创建 Client Cordis，普通用户选择任务或 Foundry Agent入口，而不是选择 renderer backend、通用 Agent host或非默认 executor adapter。
3. `Foundry Agents`
...
- 把 family capability domains 作为 OPL Framework/App/Cloud 长期设计语言和成熟度推进模型：以 `OPL Workspace` 的 `L4 executable baseline` 作为当前可执行基线，推动 `Charter / Atlas / Pack / Stagecraft / Runway / Ledger / Console / Foundry Kernel / Connect` 按各自 authority surface、Host/Client contribution、合同、CLI/App/Cloud 入口、验证、真实用户路径和 owner evidence演进，最终达到 `L5 production operating maturity`
