...
本文只维护 MAS/MAG/RCA/OMA/OBF 中仍可能被误读成 repo-local generic platform 的 surface 分类、owner 边界和迁移门。标准智能体迁移主题的当前 owner 路由见 [OPL 标准智能体迁移 owner 路由](./standard-agent-migration-owner-map.md)；ideal-operating-model redesign foldback、remaining residue/delete gate 和 active-goal baton 回到 [OPL Family 当前状态与理想目标差距](./current-state-vs-ideal-gap.md)。本文不再保存逐日拆文件流水账、line-count closeout、commit 过程、dated readout counter 或长 follow-up 清单。
...
- 当前机器兼容分类把 `mas-scholar-skills` 投影为
  `capability_package`，把 `opl-flow` 投影为
...
Standard Agent registry / alias tables 当前由 `src/kernel/standard-agent-registry.ts` 提供迁移期兼容投影。Foundry Agent CLI list/inspect、`opl connect skills` 的 domain-agent pack projection、`standard-domain-agent-conformance-foundry-agent-os` 和 target architecture `foundry_agent_os_standard.applies_to_domain_agents` 仍有 active consumer，因此不能直接删除；但它不再是长期 identity 或 membership authority。目标 identity 归 Package owner descriptor，Framework discovery 归平台 installed inventory，官方 starter membership 归 App Official Profile；迁移和 no-active-consumer 门见 [`opl-package-platform-composition-migration.md`](./opl-package-platform-composition-migration.md)。
...
MAG/RCA/OMA/OBF 的默认 owner route 由 `Codex CLI + domain stage pack` 选择，OPL 只投影 `StageRun + current_owner_delta`。OPL StageRun 只承载 attempt transport、currentness、quality-budget accounting、progress/failure diagnostics 和 owner-delta observation；不存在 execution authorization、closeout binding 或程序化 semantic route controller。
...
- `OMA` 不再保留本地 agent 构建、评测、物化或版本脚本；只保留仓库验证入口、declarative pack、专业语义 skill 与四协议 conformance fixture。是否 structural clean，回 OMA repo-native verify 与 OPL exact-checkout integration readback。
...
按严格目标态，domain repo 不能重新把 refs-only adapter、patch/materialization proxy、status/workbench/domain-action shell 或 tombstone/provenance code path 写成 generic control plane、default caller 或长期产品面。当前剩余风险按 production evidence tail、防回归和 owner authority boundary 读取，不写成 production ready。
...
| Standard Agent registry / alias tables 重复 | `compatibility_to_delete` | 当前 active consumers 先切到 installed descriptor；identity 归 Package owner，discovery 归平台 inventory，starter membership 归 App Profile。no-active-consumer proof 后删除固定 registry。 |
...
| `OMA` | 目标理解、设计依据、`AgentBlueprint` / `EvalSpec`、证据诊断与 `EvolutionProposal`。 | 已退役的本地 scaffold/评测/物化脚本、执行代理、静态晋级记录和旧 action schema。 | Fresh OMA verify 与 OPL exact-checkout integration 决定 provider shape；`opl-meta-agent` 只是 carrier slug。OMA 不得成为 FoundryRun、评测、版本、canary、activation、App/workbench 或 target truth owner。 |
...
| OMA local execution/materialization | retired agent builders、suite runners、patch/materialization proxies、bootstrap pack writers | `retired_no_compatibility_path` | OPL Foundry Kernel exact-checkout integration、四协议 purity 与 provider boundary 通过后保持物理删除；OMA 只保留 semantic provider source，不写 target truth 或 lifecycle state。 |
...
- Foundry Kernel / OMA handoff vocabulary must remain target-agent generic; domain-specific suite/command families belong in target owner refs or history.
