...
| Cordis 进程内组合 | `roadmap_complete_p1_p2_landed_p3_not_started` | P0-P6 路线图、收益、代价和 go/no-go 已冻结；P1 十模块 surface map 与 P2 隔离 Agent Executor composition 已落地。P2 只在 dev/experimental dependency graph 使用正式 `@deepseek-ai/cordis`，默认 production caller、Temporal、Package lifecycle、Ledger 与 domain/App authority 均未切换。P3 尚未开始，详细门槛见 [`cordis-adoption-plan.md`](./cordis-adoption-plan.md)。 |
...
1. `cordis_runtime_composition_adoption` 是新的战略主线。用户已确定正式
   ...
   P2 隔离 Agent Executor experiment 已落地，P3 composition inspect 尚未开始。P1 的
   ...
   不进入默认 production path，也不得把实验写成已插件化或已 Cordis-native。P3 是否启动及其
   最小写集须基于 fresh caller/readback 决定；详细验证和回退见
...
| --- | --- | --- | --- |
| Cordis runtime composition adoption | `roadmap_complete_p1_p2_landed_p3_not_started` | Framework Integrator；P3 owner/write set 待 fresh admission | P0-P6 路线图已完整冻结；P1 machine-readable surface map 与 P2 DSH scoped `@deepseek-ai/cordis` 隔离 executor composition 已落地，并证明 seam 可见性、executor 可替换、依赖注入、typed event、scope teardown 与 composition isolation。P3 尚未开始，P3-P6 的收益与 go/no-go 见 [`cordis-adoption-plan.md`](./cordis-adoption-plan.md)。当前不切默认 caller、不迁移 Temporal/Package lifecycle/Ledger/Foundry/domain/App authority、不新增第二 registry/lifecycle。 |
...
`next_action=对 P3 composition inspect 做 fresh implementation admission，确认 P2 snapshot/readback current 后冻结实际文件级 write set 并实现只读 inspect`。每轮先
