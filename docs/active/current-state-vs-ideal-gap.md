...
| Cordis 进程内组合 | `roadmap_complete_full_migration_authorized_p4_landed_p5a_active_p5b_p6_not_started` | 用户已明确授权完成 Cordis 全面迁移，迁移成本不再作为否决条件。P1-P4 已进入 canonical main；P5-A 正在迁移 Pack stage-binding、Stagecraft context 和 Runway 五个真实 caller，其他批次并行准备，P6 尚未切换默认 caller。Package currentness、Temporal、Ledger、Foundry、domain 和 App authority 继续由各自 owner 持有。详细门槛见 [`cordis-adoption-plan.md`](./cordis-adoption-plan.md)。 |
...
1. `cordis_runtime_composition_adoption` 是新的战略主线。用户已确定正式
   `@deepseek-ai/cordis` 为 OPL 长期进程内组合框架；P0 已冻结 SSOT，P1 十模块 surface map 与
   ...
   P2 snapshot/readback 并形成 deterministic read-only projection；详细验证和回退见
...
| --- | --- | --- | --- |
| Cordis runtime composition adoption | `full_migration_authorized_p4_landed_p5a_active` | Framework Integrator；P5-A successor implementation ACTIVE；P5-B..F audit/prepare | P0-P4 证据已进入 canonical main。当前实施 Pack/Stagecraft context + stage-binding successor，并准备切换 Runway 五个真实 callback caller；P5 其余批次按依赖图推进，P6 最后切换 Base/headless 与 App/托管默认入口。不得新增第二 registry/lifecycle，或越过 Package/Temporal/Ledger/Foundry/domain/App owner 边界。 |
...
`next_action=完成 P5-A Pack/Stagecraft successor，并在 fresh main 上切换 Runway 五个真实 caller`。每轮先
...
回退门禁。P5-A 仍不切默认全局入口；P6 前不得切换 Base/App 默认 caller。
