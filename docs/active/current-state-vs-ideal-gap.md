...
- Cordis 全面迁移的目标结构按 `family capability domain -> repo/product authority surface -> Package/artifact -> Host/Client Cordis contribution -> curated composition profile` 五层读取。原十品牌模块升级为跨 Framework/App/Cloud 的 capability domains，不预先决定最终源码目录、Package 或 plugin 数量；P5-R 已冻结目标图，物理 source/package cutover 已由真实 caller 和结构门完成，独立 publication 继续分账。
...
| Family capability/source topology | `physical_cutover_landed` | 原十品牌已升级为跨 Framework/App/Cloud capability domains；源码已按 13 个 responsibility source units、6 个 target roots（`authority/adapters/read-models/host/entrypoints/kernel`）完成物理重排，`src/modules/**` 为 retired/must-be-absent legacy root。该结构完成不外推 release、production、App 或 Cloud 完成。 |
| Host-derived Client Cordis / dual GUI | `top_level_contract_and_abi_aligned_runtime_e2e_admission_open` | Framework Host projection、App Client Contribution contract 与 AionUI/DSH 双 GUI 顶层 ABI 已对齐；各 GUI release/runtime 准入、跨 GUI E2E 和最终 landed 状态仍回 owner canonical source/contracts/tests/readback，本文不外推 release-ready。 |
| High-value Package topology | `workspace_source_candidates_landed_publication_open` | Runway executor、Foundry evaluation、Connect discovery、`opl-package-host` 与 Cordis ABI 已形成 workspace/source candidates 和真实 caller；只在有真实独立发布节奏、consumer和 currentness/readback时独立发布或拆仓，不机械拆十品牌或全部 plugin。 |
...
3. `family_capability_domain_and_source_topology` 已把原十品牌升级为跨 Framework/App/Cloud的
...
| Host-derived Client Cordis / dual GUI | `top_level_contract_and_abi_aligned_runtime_e2e_admission_open` | Framework Host projection + App product owner；AionUI/DSH renderer owner | Host allowlisted client graph、Client Contribution ABI、App product profile、typed slots/actions 与 RPC/events 已统一；两个 renderer 继续分别完成 release/runtime 准入和跨 GUI E2E，不建立第二 discovery/install/currentness/action truth。 |
| Family capability/source topology | `physical_cutover_landed` | Framework Integrator +各产品/authority owner | 原十品牌作为跨 Framework/App/Cloud capability domains；13 个 source units / 6 个 target roots 已完成 successor-first物理重排，`src/modules/**` retired/absent；Console分层为 Cloud product、App product与 Framework projection。 |
| High-value Package topology | `workspace_source_candidates_landed_publication_open` | Framework/Package owners | Runway executor、Foundry evaluation、Connect discovery、`opl-package-host` 与 Cordis ABI 已形成 workspace/source candidates；独立 publication/拆仓必须有真实发布节奏、consumer与 currentness/readback，不机械拆十品牌/全部 plugin。 |
