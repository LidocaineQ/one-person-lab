...
| High-value Package topology | `workspace_source_candidates_landed_publication_trigger_gated` | Runway executor、Foundry evaluation、Connect discovery、`opl-package-host` 与 Cordis ABI 已形成 workspace/source candidates 和真实 caller。Source unit 按内置模块、workspace Package、独立发布 Package、独立 repo 逐级晋升；通用性不等于 publication，publication 也不等于拆仓。当前没有满足门槛的 active publication slice。 |
...
| High-value Package topology | `workspace_source_candidates_landed_publication_trigger_gated` | Framework/Package owners | Runway executor、Foundry evaluation、Connect discovery、`opl-package-host` 与 Cordis ABI 已形成 workspace/source candidates。独立 publication 仅在仓外真实 consumer、稳定 ABI、独立版本/节奏、owner、currentness、兼容和回退证据齐备后开启；独立 repo 还需独立 owner、权限/凭据、技术栈或运维生命周期。完整长期门槛归 [`decisions.md`](../decisions.md)，当前无 active gap。 |
...
当作待完成迁移。`high_value_package_topology` 不保留常驻 publication backlog；只有发现仓外真实 consumer
和独立版本/发布节奏后，才按 [`decisions.md`](../decisions.md) 的晋升梯度核对稳定 ABI、owner、currentness、
兼容与回退，并为该 slice 冻结唯一 owner、exact write set和 affected outcome。独立发布 Package 可以留在
monorepo；只有 owner、权限/凭据、技术栈或运维生命周期也独立时才拆 repo。品牌组合只在真实 owner/caller
变化时更新，不能因命名、通用性或 plugin 数量机械拆包。
