...
| Cordis 进程内组合 | `p1_surface_map_and_p2_executor_experiment_in_progress` | P1 surface map 与 P2 隔离 Agent Executor 实验已登记并由独立 owner/worktree 推进；P2 只在实验 dependency graph 使用正式 `@deepseek-ai/cordis`，默认 production caller、Temporal、Package lifecycle、Ledger 与 domain/App authority 均未切换。详细门槛见 [`cordis-adoption-plan.md`](./cordis-adoption-plan.md)。 |
...
1. `cordis_runtime_composition_adoption` 是新的战略主线。用户已确定正式
   `@deepseek-ai/cordis` 为 OPL 长期进程内组合框架；P0 已冻结 SSOT，P1 surface map 与
   P2 隔离 Agent Executor 实验已启动。Framework Integrator 是唯一吸收 owner；P1 由
   Charter/Atlas/Pack/Runway/Connect 负责 surface 证据，write set 仅为
   `cordis-surface-map` contract/schema/focused test；P2 由 Runway/Connect 负责，write set
   仅为 root dependency manifest/lock、隔离 executor/loader adapter 与 focused test。
   P2 dependency 不进入默认 production path，也不得把实验写成已插件化或已
   Cordis-native。`docs/active/current-state-vs-ideal-gap.md` 与 security lane 的共享修改只由
   Integrator 在 fresh `main` 上语义合并；详细验证和回退见
...
| --- | --- | --- | --- |
| Cordis runtime composition adoption | `p1_surface_map_and_p2_executor_experiment_in_progress` | Framework Integrator；P1 由 Charter/Atlas/Pack/Runway/Connect 负责，P2 由 Runway/Connect 负责 | 两条独立实验 lane 已登记：P1 盘点真实 module/caller/service/event/scope/trust 边界并生成 machine-readable surface map；P2 使用 DSH scoped `@deepseek-ai/cordis` 构建隔离 executor composition。当前不切默认 caller、不迁移 Temporal/Package lifecycle/Ledger/Foundry/domain/App authority、不新增第二 registry/lifecycle。 |
...
`next_action=完成并吸收 P1 surface map 与 P2 Agent Executor 隔离实验，再以 fresh caller/readback 决定是否进入 P3`。每轮先
