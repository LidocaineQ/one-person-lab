...
supporting planes:
  Pack / Stagecraft / Runway / Ledger / Console / Foundry Kernel / Connect
```
...
理想 OPL 的体感应像一个专业团队的持续工作台，而不是一个需要用户理解 runtime ledger 的控制台。普通用户和 operator 只看到四件事：当前目标、当前 owner、这轮要产出的 delta、接受的返回形状。平台内部的 provider、attempt、lease、evidence、long-soak、cleanup、release cohort 和 L5 owner action 都在后台保持可追溯，但不抢占默认路径。
...
| `production_quality` | L5 evidence、long-soak、release/install、owner acceptance。 | module / App / domain owner。 | refs-only ledger、Owner decision projection、no-second-truth guard。 | verified ledger 等于 L5。 |
...
| --- | --- | --- | --- |
| `product_layer` | `OPL Base + OPL App + OPL Packages + OPL Cloud`；Foundry Agents 是 Packages 中的专业 authority family。 | core docs + brand system profile。 | 用户被迫理解 repo split、provider、executor selector，或把 Package/plugin/品牌模块混为一层。 |
...
| --- | --- | --- |
| `OwnerDeltaController` | 让所有默认入口只问当前 owner 欠什么。 | `framework readiness`、App fast state、operator drilldown、owner action summary 同源 current owner delta。 |
...
| `improvement_plane` | `Foundry Kernel` + `OMA` | exact-version `DesignRequest`、`EvidenceBundle`、owner feedback、no-regression refs。 | `EvolutionProposal`、next `AgentBlueprint`、new tests、risk hints。 | target-domain truth、保护测试正文、生产采用 authority。 |

这九个平面的关系固定为：ordinary progress plane 是默认用户路径；durable runway plane 只保证执行可恢复；stage artifact plane 保证结果可接力；authority decision plane 才能关闭领域进度；evidence telemetry plane 只提供可追踪性；improvement plane 产出可审核、可重新评测的完整下一版语义。
...
foundry-kernel-improvement-loop
...
| `foundry-kernel-improvement-loop` | exact baseline/candidate digest、冻结测试计划、EvidenceBundle、root cause、EvolutionProposal、risk-tier canary/activation evidence。 | target owner 的领域真相、保护测试正文、memory/artifact body 与生产采用决定。 |
...
| OMA | `AgentBlueprint` / `EvalSpec` / `EvolutionProposal` | agent-building semantics、design evidence、failure diagnosis、next-version proposal。 | second Foundry Kernel、evaluation/version/promotion owner、worktree lifecycle owner、registry/App shell、local execution growth。 |
...
| `oma_semantic_provider_hygiene` | OMA runtime surface 保持 declarative pack + protocol fixtures + repo verification；执行、物化、证据和版本实现只留 OPL。 | 新增 OMA surface 必须通过 forbidden-field、单 action、双 operation 与 no-local-execution guard。 |
