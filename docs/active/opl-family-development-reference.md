...
- provider SLO 与 repair-loop 执行证据：Temporal production proof 已有 read-model、supervised receipt 与 `family-runtime provider-slo tick --provider temporal` cadence executor；`provider_capability_slo` 已把 restart / re-query / signal history、typed closeout claim-evidence 支持、missing-closeout/no-output diagnostic 推进、retry/dead-letter transport boundary 和 domain truth boundary preservation 做成 OPL runtime/provider 机器投影。后续仍要保持 cadence/capability 长窗口持续 satisfied，并把真实 domain owner-chain dispatch 和 no-forbidden-write proof 闭合到对应 evidence gate；这些 capability checks 不参与 stage semantic routing。
...
process、carrier 和 bridge。Hermes Desktop / `hermes-codex` 已退休，仓库只保留为
read-only Git provenance，不再是候选、replay route 或维护参考；AG-UI/CopilotKit / `agui-codex` 只作为 archived technical proof 与
...
- 保持 OPL fork overlay、upstream AionUI intake、packaging/update、bridge adapter 和本地 GUI shell 规则；这些规则落在 App 合同与 `opl-aion-shell` 的实现边界，避免 AionUI 规则主导 App 顶层。Studio 的 DSH GUI/Cordis 实现只在候选仓演进；Hermes 只保留归档 provenance，AGUI archived proof 的边界由 App candidate registry / shell-adapter contracts 持有，OPL 主仓只记录消费边界。
...
OPL series 的开发文档治理不维护固定仓库数量或 allowlist；每轮按
[文档组合治理](../docs_portfolio_consolidation.md) 从 actual Git roots、repo-local owner 标识、
canonical owner 和当轮用户范围 fresh 发现实际 OPL-owned scope。当前声明 framework / domain
owner 职责的仓库采用同名 canonical docs taxonomy，其余实际纳入仓库按 repo-specific 职责
维护轻量 truth。统一目录名不是因为
