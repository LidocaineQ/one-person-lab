...
原因：Framework P1-P6 已证明 Cordis Host 的 service injection、typed event、scope teardown、composition snapshot 与受控 profile 能降低真实 caller 的 wiring 成本。Family capability-domain 与 source/package topology cutover 进一步证明，固定“十大”会在真实 owner 变化后留下平行解释：Cloud 已有独立 Fabric authority，而 Framework 十项 CLI/L4/L5 surface 只是家族能力的一部分。品牌应直接呈现真实 capability portfolio，由同一机器 registry 关联 authority、Package 与 Cordis contribution，不再维护历史数量或第二套 taxonomy。
...
- `family-capability-domain-registry.json` 是唯一家族品牌组合 SSOT。当前组合按 Foundation / Build / Run / Operate 展示 `Charter/Workspace/Atlas/Pack/Stagecraft/Runway/Ledger/Connect/Console/Foundry/Fabric` 11 个品牌；数量不冻结，只有真实 owner/caller 合并、拆分或新增时才调整。`brand-module-registry.json` 只投影其中具有 Framework CLI / L4 / L5 surface 的十项，不构成第二品牌目录。
...
原因：用户已确定 DeepSeek Harness（DSH）的“一切皆插件”是 OPL 后续最重要的学习方向，并进一步授权把 OPL 的目标终态从实验验证提升为 Cordis 全面迁移。OPL 已有十个品牌模块和多个显式 executor adapter；把真实进程内 service、event、effect 和 lifecycle 交给成熟的 Cordis 组合层，可以让有真实替换价值的能力独立演进、按需补齐并受控组合，同时不把安装、持久编排、领域判断和证据权威混进同一个手写内核。该方向是产品/架构决策，不是市场事实或当前运行状态的断言。
...
- Framework 提供统一的 `opl-package-host` 兼容服务，而不是要求每个 Package 重复声明 Framework 内部 Cordis plugin 清单。`opl_agent_package_manifest.v1` 默认映射为 standard-agent runtime，`opl_capability_package_manifest.v2` 默认映射为 capability provider，`opl_workflow_profile_package_manifest.v1` 默认映射为 workflow-profile source；MAS/MAG/RCA/OMA/BookForge、MAS Scholar Skills/OPL Persona/OPL Relay 与 OPL Flow 因而共享同一 host context ABI。它们仍允许 standalone；托管启动时 required provider 缺失才阻断，optional provider 缺失只降级诊断。
- Package host context 必须绑定最终 composition profile、root/child snapshot digest、provider API/scope 和 disposer；`app-full`、`foundry-dev` 不得继承错误的 `base-headless` identity。该 context 是一次托管调用的不可变装配输入，不是 Package installed/currentness、domain quality、owner receipt、Temporal history、Foundry activation 或 App product truth。
- P5-R 已执行 `Cordis-native architecture re-baseline`；其四层运行图已由 2026-08-15 决策扩展为 family capability domain、authority surface、Package、Host/Client Cordis contribution 和 curated composition profile 五层。品牌组合由唯一 registry 随真实 authority/caller 演进；物理边界依据 authority、caller、lifecycle、scope、trust、故障隔离、发布节奏和替换价值，而不是目录对称、历史数量或命名偏好。
...
- 长期生态模型固定为 `OPL Base ≈ R`、`OPL App ≈ RStudio`、`OPL Package ≈ R
  Package`。Package 是唯一安装单元；Skill、Tool、Plugin、MCP、workflow 和 entrypoint
  是 descriptor 中可发现 capability，不形成平行 lifecycle。Package identity、carrier
  和 executor route 是三个正交维度，任一维度都不得成为另外两个维度的 authority。
...
- 稳定 Package/capability/entrypoint identity 只允许向后兼容扩展。breaking interface
  ...
  版本 resolver。
- 每个 first-party Package owner 在独立 GHCR repository 发布完整官方 Package runtime，
  独立推进自己的 `latest-stable`。共享
  `one-person-lab-manifest:latest-stable` 只保留 Full/offline/integration/QA 快照角色，
  退出普通安装、更新和 currentness。
- Framework 目标只保留薄 OCI download/verify/bytes-handoff adapter，以及从 OCI、
  Git/local、offline 和其他 native carrier 取得 fresh readback 的 carrier-neutral
  installed aggregation；配置的 carrier 或 Package runtime adapter 执行
  install/update/remove。完整 runtime bytes 缺失时必须报告 `physical_unavailable`。
- Codex Plugin Manager 是首个 carrier adapter，只持有 Codex plugin/config/cache
  投影，不是 Package identity、installed truth 或生态 authority。完整 Package runtime
  不得被 Plugin 子集替代；App metadata、descriptor 或残留 cache 不得伪造 installed。
- executor adapter 只持有 route callability。切换 Codex CLI、Claude Code、Hermes 或
  后续 executor 不得重装 Package，也不得丢失 Settings/Home preference、required
  dependency presence、Work Item 或 typed view；某 adapter 缺失只局部降级该 route。
- Framework 另保留 installed descriptor discovery、presence/callability、Agent Work
  Item/Temporal execution 聚合和 typed custom-view proxy，不持有跨包版本求解。
...
- 2026-08-11 追加：Standard Agent descriptor 可在 `workspace_binding.project_collection_path` 显式声明 domain canonical collection；未声明时仍使用 OPL 默认 `projects`，所以 RCA/MAG/OMA/OBF 无需迁移。MAS 声明 `studies` 时，OPL initializer 物化 `studies/<study-id>`，保留 Framework 内部 `projects[]` ledger，并按 descriptor `inventory_projection` 在同一 `workspace_index.json` 根层投影 `/studies`。OPL 只能初始化和同步 identity/root 结构字段；status、stage、package、lifecycle 和 next-action 继续由 domain owner 产生，`workspace ensure` 必须保留现有 domain rows 及其业务字段。明确已选择或已安装的 Standard Agent descriptor 无效时，workspace 写入入口 fail closed，不再静默回退 `one_off/projects`；真正未安装且未选择 descriptor 时保留 generic fallback。已有 workspace 的 readback 使用 index 中冻结的 agent/profile 结构，不得因当前安装漂移而变得不可读。
