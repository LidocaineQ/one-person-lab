...
`Verify` 的 local structure gate 可以在 Sentrux drift 时输出 `opl quality details` 作为 triage 片段，但该诊断必须受 `OPL_QUALITY_DETAILS_TIMEOUT_SECONDS` 约束，不能耗尽 hosted CI 预算。Framework 的完整 hosted `Verify` 只按每日 schedule 或手动 dispatch 运行，不为每次 push/PR 重复启动，也不再维护独立 `Sentrux Advisory` workflow；完整 markdown / JSON 详情按需从本地命令或显式消费 `.github/actions/quality-details` 获取。若 local structure gate 的详情超时，记录 warning 并继续按 advisory 结构口径处理。
