...
State: `active_policy`
Machine boundary: 本文是人读维护政策。可执行守门继续归 `contracts/opl-framework/reuse-first-governance.json`、`scripts/reuse-first-scan.mjs`、`./scripts/verify.sh reuse-first`、source、tests 和 runtime/readback surface。
...
3. **结构证据不能越权成 ready claim。** docs、contract、focused tests、line split、fixture/live-test proof、readback clean 和 clean diff gate 都不能声明 runtime ready、domain ready、release ready、production ready、owner acceptance 或 physical delete authorization。
...
| 经验类型 | 默认落点 | 不应落点 |
| --- | --- | --- |
| 当前执行进度、fresh evidence 和完成度 | 唯一 active owner `docs/active/current-state-vs-ideal-gap.md` 或对应 owner surface | 新建平行 closeout 文档 |
| 可执行 strict diff gate | `contracts/opl-framework/reuse-first-governance.json` 与 `scripts/reuse-first-scan.mjs` | 只写 prose checklist或历史扫描 worklist |
...
- 对 reuse-first finding 的处理优先级是：替换 active caller、迁移到共享 owner boundary、收敛为 generated / hosted surface、记录明确 owner decision、最后才允许保留 projection boundary；执行状态回现有 owner surface，不写回扫描器。
