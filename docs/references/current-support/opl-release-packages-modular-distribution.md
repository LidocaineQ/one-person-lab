...
模型。Package identity、carrier 与 executor route 相互独立；依赖只检查
presence/callability。

每个 first-party Package owner 在独立 GHCR repository 发布完整官方 runtime，并独立
推进自己的 `latest-stable`。Base 只保留薄 OCI download/install/update adapter 和
carrier-neutral fresh installed aggregation；Codex Plugin Manager 只管理 Codex
plugin/config/cache 投影，不是 Package identity、installed truth 或生态 authority，
也不能用 Plugin 子集替代完整 Package runtime。共享
`one-person-lab-manifest:latest-stable` 只用于 Full/offline/integration/QA 快照，不参与
普通安装、更新或 currentness。executor adapter 缺失只局部降级对应 route；切换 executor
不得重装 Package 或丢失偏好、任务、依赖和 typed views。
