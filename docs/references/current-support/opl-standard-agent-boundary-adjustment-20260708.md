...
| --- | --- | --- | --- | --- |
| MAS (`mas`) | 医学研究 stage、source/evidence refs、论文 package、医学质量 gate。 | queue、attempt transport、runtime env、workspace/source/artifact locator、App/operator projection、owner-evidence transport。 | study truth、publication quality verdict、artifact/package authority、medical memory accept/reject、owner receipt、typed blocker、human gate、MAS stage prompt、MAS Scholar Skills truth。 | Codex CLI 独占语义 route；OPL 不再持有 autonomy supervisor、transition packet 或 decision ledger。 |
...
| OMA (`oma`) | target-agent create/takeover/improve semantic provider。 | OPL FoundryRun、Pack materialization、Runway evaluation、Ledger evidence/version、Owner gate、canary、activation/rollback 与 generated interfaces。 | 目标理解、`AgentBlueprint` / `EvalSpec`、证据诊断与 `EvolutionProposal`。 | OMA 身份为 `oma / agent_engineering / opl-meta-agent`；无本地执行、物化、评测、版本或 promotion state。 |
...
| MAS Scholar Skills (`mas-scholar-skills`) | 外部 professional capability package 与 package-owned capability refs。 | OPL generic package spec/channel、plugin/Skill validation、target-bound activation 和 provenance receipt。 | plugin manifest、Skill 清单、内容合同、正文和专业判断。 | 不是 standard domain agent；OPL-local catalog/profile/plugin mirror 已退役，当前能力只从 canonical package source 读取。 |
...
| 旧 supervisor CLI / parser | 第二语义控制面 | 已物理退役；普通推进直接由 Codex 选择 declared stage。 | 不恢复 alias、parser、ledger 或 compatibility wrapper。 |
