...
`src/modules/<module_id>/` 是 Framework 源码 owner。`OPL Cloud`、在线 Workspace、Console 页面、Gateway/API 和 `OPL Fabric` 是正在落地的 Cloud 产品语义，可以组合多个 Framework 模块形成用户可见能力；具体能力以真实 account、storage、isolation、backend、owner policy 与运行证据为准。它们不作为第 11 个 Framework 源码模块。`OPL Fabric` 的通用资源底座能力由 `Connect`、`Runway`、`Pack`、`Workspace` 和 `Ledger` 协作承接。
...
2026-08-11 的当前主线收口修复了后来重新形成的 `atlas / connect / runway / workspace` SCC：Attempt-bound Skill projection 与 Standard Agent framework-import inspection 下沉为 kernel primitive；Package descriptor、checkout、progress delta 通过 Connect 注册的 readiness port 消费；Workspace binding 通过 Workspace 注册的 binding port 消费。源码方向稳定为 Connect 可以编排 Runway / Workspace / Atlas，而 Atlas、Runway、Workspace 不再反向 import Connect，Atlas 也不再反向 import Workspace。四条反向方向已进入 forbidden policy，dependency-cycle policy 默认 strict；fresh readback 仍须同时证明 `deep_import_violations=0`、`forbidden_dependency_violations=0`、`dependency_cycles.count=0`。端口只倒置源码依赖，不转移 Connect 的安装真相、Workspace registry、Runway runtime 或 Atlas catalog owner。
