...
OPL Framework 当前由十四个品牌模块构成。它们不是把旧系统机械切成十四份，而是按照“什么值得独立理解、独立替换和独立承担责任”重新划定边界。
...
这十四个名字既是对外可理解的产品语言，也是内部架构的直接边界。它们共同构成 OPL Framework 的品牌，不再另外维护一套只用于讲故事、却与实现分离的“十大模块”。

模块数量也不是需要永远维护的象征。如果未来两个责任确实合并，或一个责任需要进一步拆分，品牌名、源码边界、发布单元和运行单元应当一起变化。OPL 追求的是概念与实现长期一致，而不是保住一个漂亮但失真的数字。

## 一个名字，对应一个真实单元

很多软件会同时存在产品模块、源码目录、发布包和运行插件四套分类。它们各自看似合理，叠加后却让维护者必须不断翻译：一个品牌名究竟对应哪些代码、哪个版本、哪份发布物和哪个运行实例？

OPL Framework 选择更直接的原则：**一个品牌模块，对应一个主要源码边界、一个 Package 身份、一个 Cordis plugin 身份和一条独立版本线。**

这不是要求每个模块都立即拥有独立仓库，也不是鼓励用户任意拼装所有组合。仓库只是协作载体，边界是否真实取决于模块能否独立发布、验证、替换和追踪。高变化、高替换价值的模块可以先拥有独立发布节奏；稳定模块可以继续共处于同一仓库，但不能失去自己的身份。

对开发者，这意味着看到模块名就能找到它的源码、测试和版本。对运维者，这意味着一次故障可以落到明确的模块与组合快照。对用户，这意味着产品叙事不是包装，而是系统真实结构的可读表达。

这是一条明确的目标边界，也是一项需要诚实推进的工程工作。Framework 已经形成十四个 landed Cordis contribution identities；在同一仓库内承载多个贡献的过渡阶段，仍可能存在。独立 Package、独立发布节奏和独立仓库会优先落到高替换价值的 Runway Executor、Foundry Evaluation、Connect Discovery 与 Package Host 等单元，再由真实 caller、版本和发布证据决定下一步。白皮书展示的是统一的最终认知，不把尚未完成的拆仓或公开发布写成既成事实。

## 自由组合，不把复杂度交给用户

可组合不等于让用户面对十四个开关和难以验证的组合矩阵。OPL Framework 在内部保留模块自由，在外部提供少量经过验证的组合形态。
...
十四个品牌模块让规则、发现、工作空间、阶段、执行、证据、呈现、评估与连接拥有真实边界；Cordis 让这些模块能够组合、检查和替换；受控 Profile 让内部自由不会成为用户负担；Framework Host 与 App Client Cordis 让运行和界面共享同一种设计语言；Foundry 则让每项能力可以在证据约束下持续改进。
