...
**OPL Framework** 是运行底座和组合主机。它负责显式启动、阶段推进、长期运行、恢复、工作空间、证据与交接，也决定一次运行实际装载哪些经过批准的能力。具体执行工具、能力实现和界面可以变化，工作本身的阶段、成果与责任关系保持稳定。

**One Person Lab App** 是用户工作面和产品体验的负责人，让用户选择任务、查看进度、打开文件、处理卡点和接收更新。OPL 可以使用不同的界面实现：当前 AionUI 主线与 DeepSeek Harness GUI 候选共享同一套产品能力、操作和状态来源，差异只在呈现与载体。**OPL Cloud** 把同一条工作线扩展到在线工作空间、托管资源、组织管理和协作场景。App 与 Cloud 都沿用 Framework 的运行事实和领域 Agent 的专业结论，不另造一套相互冲突的状态。
...
OPL 把稳定的产品认知与具体实现分开。Workspace、Runway、Ledger、Console、Connect 等名字描述的是整个 OPL 产品族中的能力领域，不要求每个名字都对应一个代码目录、一个安装包或一个插件。真正的事实、权限与产品决定仍留在负责它们的 Framework、App、Cloud或专业 Agent；可替换的实现则通过显式接口组合。

Framework 使用 Cordis 组织这些运行能力，并为 App 提供一份受控的界面能力清单。App 的界面在这份清单上组合视图、动作和状态，所以更换 AionUI 或 DSH GUI renderer 不需要复制插件目录、安装状态或产品规则。界面不能自行发现或安装能力，也不能凭显示结果改写运行、专业质量或发布事实。

这种分离带来三个直接收益：修改执行器、连接器、观察器或界面时，改动不必穿透整条运行链；高替换价值的能力可以独立版本、测试和回退；用户面对的是少量经过设计的产品组合，而不是任意插件矩阵。独立发布只用于确实有不同升级节奏的能力，不为了结构整齐把所有模块机械拆包。
