...
**OPL Base** 是运行底座，由 OPL Framework 实现。它负责启动、阶段推进、长期运行、恢复、工作空间、证据与交接，让复杂任务可以被启动、暂停、继续和收口。

**OPL App** 是本地工作面，让用户选择任务、查看进度、打开文件、处理卡点和接收更新。不同界面实现遵循同一份产品定义和工作语言。
...
Base 管共同底座，App 管本地体验，Packages 管专业能力，Cloud 管在线服务。前三类以本地软件方式安装和更新，Cloud 通过在线产品持续交付；四者采用各自合适的交付方式，共同维持一致的任务、成果、证据和责任语言。
