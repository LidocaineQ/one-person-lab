...
`OPL` 的目标不是只做入口聚合或工作台投影，而是完整的 stage-led family agent runtime framework。对外产品认知固定为 `OPL Base + OPL App + OPL Packages + OPL Cloud`：Base 提供运行基础，App 提供本地工作台，Packages 提供可安装专业能力，Cloud 提供在线治理、托管资源与协作服务。Foundry Agents 是通过 Packages 交付并保留领域权威的专业能力家族，不是第五个产品层。OPL 已完成向 DSH 正式 `@deepseek-ai/cordis` 的 Framework Host 全面迁移：`base-headless` 是默认 composition，P1-P6、真实默认 caller、snapshot/digest、child fiber teardown 与显式 services 均已落地。Family capability-domain rebaseline 与 Package/source topology cutover 已把品牌认知、authority owner、Package 发布单元、Cordis contribution 和源码单元解耦；当前两个 workspace Package 仍是 source-extracted candidates，不等于已经独立发布，Runway executor、Foundry evaluation、Connect discovery 已作为 Host-owned Cordis contribution 内联。Cordis 只负责进程内 composition；Package/native carrier、Temporal durable history/retry/replay、Workspace bytes/binding、Ledger receipts/evidence、Foundry activation、domain truth 与 App/Cloud product truth 仍归各自 owner。阶段内最小执行单位是 Agent executor；`Codex CLI` 是当前第一公民 executor。

### 2026-08-15：Family capability domain 与 Host/Client Cordis SSOT

品牌不再由历史“十大模块”定义，也不再复制 Framework 目录、Package 或 plugin 清单。`contracts/opl-framework/family-capability-domain-registry.json` 是唯一家族品牌组合 SSOT；当前组合是 `Charter`、`Workspace`、`Atlas`、`Pack`、`Stagecraft`、`Runway`、`Ledger`、`Connect`、`Console`、`Foundry` 和 `Fabric`。它们按 Foundation / Build / Run / Operate 四段用户旅程展示，并直接绑定真实 authority surface、Package 与 Cordis contribution。品牌数量不冻结：只有真实责任和 caller 发生合并、拆分或新增时才随之调整。`brand-module-registry.json` 只是其中具有 Framework CLI / L4 / L5 surface 的投影，不构成第二品牌目录。
...
同一顶层设计同时约束 AionUI 主线和 DSH GUI 候选：二者共享 Host 投影的 client graph、Client Contribution ABI、App product profile 和 action/slot semantics；只有 renderer、carrier 和视觉实现可不同。通过同一 compatibility、E2E 与 release/runtime admission 后，App 才能在二者之间切换；这不是绕过验证的任意热切换。Cloud 的 Console 是托管 control-plane 产品，App 的 Console 是本地产品工作面，Framework 的 Console surface 只提供 readiness/operator/action projection 和 read model，三者不得互相冒充 authority。
...
桌面 GUI 不是 Framework 的第二个产品，也不是 Node 直接渲染 React。Packages 作为独立
生态层向 Host 贡献 runtime capability，也可通过受限 schema 向 App 贡献 GUI 描述；正确
关系是：

```text
OPL Packages -> Framework Cordis Host（runtime/plugin contributions）
             -> App-owned GUI ABI（declarative GUI contributions）
...
每个 first-party Package owner 在自己的 `ghcr.io/<owner>/one-person-lab-packages/<canonical-id>` repository 发布完整官方 Package runtime，并独立推进自己的 `latest-stable`。OPL Base 只下载、校验并交付 OCI bytes；配置的 carrier 或 Package runtime adapter 执行 install/update/remove，并提供 native readback。Framework 聚合 OCI、Git/local、offline 和其他真实 carrier 的 fresh installed/callable 状态，不再维护中央 resolver、installed lock、Package payload、LKG、lifecycle receipt、materializer、scope activation 或 durable Package transaction。`ghcr.io/<owner>/one-person-lab-manifest:latest-stable` 只服务 Full、离线、集成测试和 QA 的组合快照，不定义普通 Package currentness。
...
release manifest/checksum/attestation、Temporal Worker Versioning 和 domain artifact/evidence digest 仍由各自 owner 保留；它们不属于 Package lock，也不得成为普通安装或运行门禁。完整迁移、功能等价和物理删除门见 [`docs/active/opl-package-platform-composition-migration.md`](./active/opl-package-platform-composition-migration.md)。Package topology cutover 已为 Runway executor、Foundry evaluation、Connect discovery、`opl-package-host` 和共享 Cordis ABI 建立独立 workspace/source candidates 与真实 caller；下一阶段只按真实 consumer、安装/发布节奏、currentness/readback 和回退价值决定是否独立发布或拆仓。不会按 plugin 或品牌数量机械拆成 Package。当前 `opl packages` 只消费 native carrier descriptor/readback；历史 lock/receipt/LKG 仅作为 provenance，不能成为新的 authority。
...
以下是十个 family capability domains 的代表性 Cordis 贡献。它不是最终 plugin registry、Package 清单或源码目录；一个 domain 可以贡献多个 Host/Client plugin，多个 domain 也可以合并到同一受控 contribution，没有独立 caller、责任或替换价值的 surface 应合并或降为 read-model/authority。
...
Cordis plugin API、源码版本、Package 版本与 composition snapshot 分别标识；开发分支只承载候选，不是生产组合机制。只有有独立发布/安装节奏且替换价值足够高的 capability 才提升为独立 Package，且只有真实 publication 与 owner channel readback 完成后才称为独立发布。组合通过显式 `provides/injects`、API compatibility、scope、trust 和 Host-to-Client projection contract 校验，不建立中央版本 resolver。required plugin 缺失或不兼容时，composition fail closed；optional plugin 只能形成诊断/降级，不得写 domain/App/Cloud verdict。生产 attempt 和 Client graph 都在启动或挂载时冻结 composition snapshot，运行中不得 hot swap；HMR/reload 仅限 dev/experimental scope。Cordis 不是安全沙箱，不可信 plugin 必须由现有 sandbox/provider 或独立进程隔离。

Cordis adoption 的详细阶段、owner、写集、验证、完成门和回退见 [`docs/active/cordis-adoption-plan.md`](./active/cordis-adoption-plan.md)。P1-P6 的 Framework Host 默认路径、legacy WorkspaceBindingPort caller-zero、family capability-domain registry、13 个 source units / 6 个 target roots 和两个 workspace Package 候选已进入当前源码；Runway executor、Foundry evaluation、Connect discovery 的 Host-owned contribution 不再形成独立 workspace Package。AionUI 与 Studio 的 release/runtime 准入、独立 Package publication 和跨 GUI E2E 仍须由各自 owner 的 canonical readback 证明。既有 Package lifecycle、Temporal、Foundry、Ledger、Cloud、domain 与 App product authority 继续由各 owner 保持。

### Legacy Package Manager compatibility inventory

本节只解释迁移前机器行为和 retained consumer，不定义新设计。以下文字中涉及 Package registry、materialization、lock、receipt、LKG、固定 first-party cohort、Release Set 或版本/ABI gate 的“固定”“必须”“唯一”等措辞，均按 compatibility inventory 阅读，不得成为新 consumer、writer 或 readiness gate。非 Package 的 StageRun、domain authority、release artifact integrity 和 user-owned file safety 边界继续有效。

标准 Agent 的实现 profile 只有一份机器真相：`contracts/pack_compiler_input.json#/implementation_profile`，由 OPL Pack compiler、scaffold validator 和 materialization path 消费。它把 identity 固定为 declarative Markdown/JSON pack，允许 helper 按 domain 需要增减但不让 helper 语言改变 Agent 身份；generated surfaces 与 Framework runtime 仍归 OPL，Framework helper 优先复用 Node standard library。MAS、MAG、RCA、OMA、OBF 同级消费这一 profile；ScholarSkills 仍是 framework capability package，不得套用 standard domain-agent identity。人读规范与五仓参考矩阵见 `docs/specs/standard-domain-agent-implementation.md`。

标准 Agent 的 hosted interface 采用同一 owner split：domain repo 在 `contracts/domain_descriptor.json#/standard_agent_interface` 或 repo-contained JSON pointer 中声明 workspace defaults/locator、runtime domain/registration ref、progress aliases 和 routing signals；OPL 只持有 `standard-agent-interface.schema.json`、closed-object parser、generated scaffold 和通用 consumer。Connect 先按 module source policy 选出唯一 source channel：普通用户跟随 owner 的 `latest-stable`，Developer Mode 或 path override 跟随可信本地 checkout；每个 hosted action 和每个新 child Attempt 绑定一次可运行的 immutable runtime input，刷新/admission 失败时只按 hosted-runtime contract 处理，不恢复 Package Manager 的 lock、receipt、materializer 或 LKG authority。Git/digest/version/currentness 只作 runtime provenance；显式 `OPL_FAMILY_WORKSPACE_ROOT` 只作为开发/验证 source selector 输入。绑定的 runtime input 非法或必需 ABI/export/module/Skill、路径/权限/安全、health/handler probe 失败时 fail closed；App 跨 Agent 聚合只把该真实 hard stop 隔离为 unavailable diagnostic。descriptor 不可用时，Workspace 采用统一 `one_off + standard_agent_workspace + project` baseline；可执行 action/stage 必须通过 OPL hosted binding 进入，缺 binding 时 fail closed。progress 只识别 `deliverable_progress_delta` 与 `platform_repair_delta` 两个 canonical 字段，不推断 paper/grant/visual alias；不得回退到 OPL registry 中的 MAS/MAG/RCA/OMA/OBF 私有命令、workspace profile 或领域 stage-log 字段。

`src/kernel/standard-agent-registry.ts` 因此只持有 identity、公开 label、package/repo/plugin/module locator、aliases 与 series membership。领域 golden path、Pack 示例、authority kernel 示例、workspace profile、runtime registration、installer/source carrier 都不再由 registry 定义；capability package 集合与 standard-agent 集合按 membership 派生，具体 Package lifecycle 归 installed descriptor、owner channel 和 native carrier readback，历史 lock/receipt/LKG 只作 provenance，不构成 Framework authority。主模块是 Atlas、Workspace、Runway、Connect 与 Pack，协同模块是 Stagecraft、Ledger、Console 与 Foundry Kernel；这条结构不把 domain truth、artifact/memory body、质量/导出裁决、owner receipt、typed blocker、App release authority 或 AionUI body 上收到 OPL。
...
生命周期架构仍按 owner 分层，但不再由 Package Manager 统一编排：`opl_base` 由 `opl update` 驱动并只管理 OPL Base 自有 runtime/provider；`opl_app` 只投影 App carrier 的 `host_update_route` 与 `host_executor_required`，实际更新归 App/host；`opl_packages` 只通过 installed descriptor、owner channel 与 configured native carrier 处理 Package-local install/update/remove/repair，并以 fresh carrier readback 判定 installed/callable/currentness。`opl packages` 不读取 root package-lock，不维护跨 Package dependency transaction，也不把 materializer、lifecycle receipt、LKG 或 rollback state 当作普通 Package authority。`opl connect sync-skills` 是显式的 companion carrier projection，复用同一 owner descriptor，但不与 Package action 共享第二套 marketplace/currentness ledger；用户删除后 native carrier readback 继续决定 `physical_unavailable`，不能由 cache 或 metadata 伪造 installed。Base runtime 自己的 staged root/current/rollback pointer、Release Set 的 digest/receipt 和 domain-owned evidence 仍由各自 owner 保留，不能反向恢复 Package Manager。

required capability package 仍是 descriptor 声明的 required/optional dependency，不是第四层产品。以 MAS 为例，用户只安装 `mas`；Framework 通过 installed descriptor 和 owner adapter 确认 `mas-scholar-skills` 的 required presence/entrypoint callability，明确的 `opl connect sync-skills` 或 Package-local repair 才能更新 Codex companion carrier。`.agents/skills`、`.codex/skills` 和 plugin marketplace 都只是 carrier/readback surface；它们不生成 Package identity、跨 Package currentness、LKG 或 transaction。一次 hosted action 绑定自己的 runtime input 后，后续 Package carrier 更新不撤销既有 attempt、artifact 或 evidence；缺 required dependency 时只阻断该 root，不连坐 Base、App 或无关 Package。
...
`OPL Runway` 的 Temporal `StageRunWorkflow` 是非模型、durable 父级 controller，不再是 `StageAttemptWorkflow` alias。它运行时动态创建 `producer -> reviewer -> (repairer -> re_reviewer) x 0..3` child workflows；Attempt role 是 Framework 有限枚举，domain 只提供专业 prompt、rubric、必要顺序和 route-back 语义。每个 child workflow 启动新的 `codex exec` 和独立 thread/session，只通过 exact artifact/source/rubric refs、finding、repair map 与 lineage 通信。role prompt 同时绑定 backing file 与实际注入 Markdown section 的 byte digest。`parent_attempt_ref` 记录上一质量角色，`artifact_producer_attempt_ref` 独立记录当前 artifact bytes 的真实生产 Attempt：初审指向 producer，repairer 延续当前生产者，re-review 指向实际产出本轮新 artifact identity 的 repairer；repairer 没有产出新 artifact 时保留旧可消费产物并终止为质量债，不启动伪 re-review，也不生成伪 receipt。same-thread 自检只能记为 `in_thread_refinement`；缺 typed closeout 时的 same-thread resume 只能记为 `protocol_closeout_resume`。该 resume 强制 read-only，只补既有 Attempt 的 JSON closeout，返回包通过 Attempt identity 校验且期间没有 command / unsupported function event 后才记成功；两者都不是正式 Review、不产生 review receipt、不消耗质量修复轮次。通用 ResumeSignal / operator resume 对 StageRun quality child fail closed，只有 controller 可以物化新的 Attempt。

StageRun controller 只拥有同一 StageRun 内的 Attempt 拓扑，不拥有跨 Stage 专业路由判断。primary-only StageRun 由 producer 作为终局 Codex Attempt 选择 declared next Stage 或 route-back；启用正式 Review 时，只有 reviewer / re-reviewer 承担终局选择，repairer 只能返回修复结果和 route recommendation。通常 `repair_required` 在预算内继续本 Stage 的 repair；若 reviewer / re-reviewer 明确判断 required repair 属于另一个 declared Stage，则可用 `repair_required + route_back` 提前终止当前质量循环，未关闭 findings 保留为质量债。该例外不适用于当前 Stage、自称 advance/repeat/skip/reverse/complete、producer、repairer 或 hard stop。Attempt 的 closeout route output 是一次语义判断结果，不把 Stage graph、current pointer 或 transition authority下放给 Attempt。OPL 必须先校验 Attempt role/终局资格、route shape、decision/recommendation 互斥、legacy 字段缺失、finding-closure 和 declared target，再由 controller 物化通过校验的 Codex route。校验失败只删除无权 route output、记录 route quality debt并保留可消费 artifact；fallback 只沿 action ordered required route，或在没有 action route 时沿唯一 declared successor，绝不按 manifest 文件顺序选择分叉。这属于 ABI/authority guard，不是对专业路线的批准、否决或替换，也不恢复已退役的 Stage Transition Authority。

StageRun 的创建入口只有 pack-bound path：普通 `opl family-runtime attempt create` 从已安装或显式 domain pack 编译 `stage_quality_cycle_policy_ref`、四类 role prompt、rubric、source/goal/lineage refs 和 manifest SHA，生成 `opl_pack_bound_stage_quality_runtime_binding` 后才可启动 `StageRunWorkflow`。`stage_run_id` 只绑定 domain、stage 与 durable invocation；manifest、prompt、rubric、policy、source、checkpoint、input artifact 与 root package closure 的实际 content digest 进入 `stage_run_spec_sha256`，普通 ref 和波动 currentness observation 不构成 immutable binding。pack 外 source/checkpoint/artifact 必须带可信 authority root 下、文件名绑定 receipt bytes 的 content-addressed identity receipt。每个新 child workflow 都携带版本化的父 spec binding，在 Attempt 物化以及真正进入 executor 前分别 fresh 回读 bytes，并确认实际注入的 Stage/role prompt 正文仍与父 StageRun 一致。

Runway 在 Temporal start 前把 exact input 登记到 `stage_run_launches`，再以带 lease 的 `BEGIN IMMEDIATE` compare-and-swap 把 `registered|start_failed|expired starting` 收敛为唯一 active starter。`starting` 表示 provider start 结果仍未知；崩溃恢复可以用同一 deterministic `workflow_id` 重投 provider RPC，所以 RPC delivery 是 at-least-once，而 Temporal `USE_EXISTING + REJECT_DUPLICATE` 保证同一 StageRun 只物化一个 execution。`started` 不得被晚到 failure 降级，`closed` 不得被晚到 receipt/failure 重开；任何状态下，同一 workflow id 出现不同 first-execution identity 都必须 fail closed。raw `family-runtime stage-run start` 已退役；`stage-run` CLI 只提供 query。producer / repairer 的本地 artifact 由 OPL transport 分块读取稳定的最终 bytes、重算 SHA，并生成 content-addressed `opl_transport_artifact_identity_receipt`；外部 artifact 由可信 domain authority receipt 提供 owner 绑定。每个 reviewer、repairer、re-reviewer 还会校验当前 artifact bytes、receipt 和 `artifact_producer_attempt_ref`，防止 closeout 后篡改或错误 lineage。review receipt 再绑定 exact artifact refs/hashes、fresh session、rubric、finding lineage 和 controller verdict。Temporal workflow 行为变化使用 `patched` marker，并以旧 Event History fixture 做 replay gate；pre-marker child 已记录的 completed event history 可沿冻结 Workflow 分支 replay，但缺 content-binding version 的旧 Activity re-execution / retry 会由当前 resolver fail closed，该分支也不能接纳新 child workflow。
...
Meta Review 是独立顶层 StageRun，不继承被审查 Stage 的生成对话，也不递归套一层正式 Stage Review。MAS、MAG、RCA、OMA、OBF 统一采用该顶层 Meta Review；其他 OPL Agent 是否启用由 domain policy 决定。普通用户投影只显示当前 Stage、Stage artifact、completed / quality debt 和 next Stage；attempt role、round、thread/session、artifact identity receipt、review receipt、finding/repair lineage与 token/cost 只进入 developer/operator drilldown，不能显示为“小 Stage”。主模块是 `OPL Runway`；协同模块是 `Stagecraft`、`Pack`、`Ledger`、`Console` 与 `Foundry Kernel`；不触碰 domain truth、专业 Review 结论、artifact body、owner receipt、typed blocker 或 Stage transition authority。

Handoff 不按 Stage 名称统一套 Review，而按新风险判断。Packaging Stage 在 manifest 把 artifact effect 分为“已审 immutable refs”“已审 bytes 的确定性机械封装”“需要专业判断的新/转换后交付 bytes”，并声明是否冻结 canonical bytes、签发 quality/export/publication/ready claim、由谁保留 acceptance。产生 reviewable bytes、冻结或签 ready claim 任一成立时，`OPL Pack` 编译器要求 `formal_review.required=true`；前两种低风险 effect 且 acceptance 留给下游 owner 时允许 primary-only。这个机器判据只决定是否需要上下文独立的质量判断，不规定领域内工具和认知步骤，也不让 human gate 替代专业 Review。
...
`Stage Transition Authority`、transition intent/event ledger、domain-semantic accept/reject decision 与 `DomainProgressTransitionRuntime` 已物理退役。当前链路只有终局 Codex Attempt 的 `stage_route_decision`、非终局 Attempt 的 `stage_route_recommendation`、StageRun identity、provider attempt、passive ledger 与 transport readback。OPL 不生成可批准、否决、覆盖专业路线的 authority event，但必须拒绝非终局角色越权、非法 shape、legacy route 字段、undeclared target 和无效 Re-review closure。MAS/MAG/RCA/OMA/OBF 的 domain pack提供 artifact、diagnostic、owner answer、typed blocker、human gate decision、专业 route 语义与 declared default progression；Framework 只做 ABI guard 和 transport。
...
物理重排的收益门不是目录整齐，而是开发者能从路径直接判断 authority、adapter、read model 或 Host contribution，且修改一个 seam 不需要理解整个品牌桶。迁移后的 public interface 由对应 authority/adapter/read-model/host package export 与 machine-readable contract 构成；source-unit 恰好一次归属、跨边界 deep import、forbidden dependency、cycle 和 legacy-root absence 是确定性门禁。当前源码已达到这些结构条件；独立 Package publication、运行准入和各产品 release/production readback 仍是独立后续边界。
...
| `OPL Foundry` | 内部 Foundry Kernel 承担 Agent 创建、接管、改进、确定性物化、独立评测、EvidenceBundle、AgentVersion、canary、activation 和 rollback。 | DesignRequest、AgentBlueprint / EvalSpec、evaluation evidence、Owner decision receipts。 | MAS/MAG/RCA/OMA/OBF 的 domain authority 与保护测试正文。 |
...
| `OPL Fabric` | Cloud 托管资源供给、provider operation store 和 resource readback。 | Cloud account、provider、storage、isolation 和 owner policy。 | Framework Host、App release、Package currentness、domain verdict。 |
...
当前品牌组合是产品与责任导航层，不等于以上五层的一对一映射。重基线的判断顺序是：
...
Framework projection 的依赖读法是：`Charter` 固定语言和边界，`Atlas` 提供目录、capability registry catalog 和 tool-card catalog，`Workspace` 提供可检查落点，`Pack` 固定 domain pack / capability registry ABI / Agent Tool Arsenal / authority ABI / generated-surface 输入，并通过 Pack OS 承载 capability pack descriptor、install registry、content-addressed cache、refs-only distribution、refs-only lock、artifact lifecycle refs 和 review receipt refs，`Stagecraft` 设计 stage 内专家工作、capability use policy 和 tool affordance boundary，`Runway` 承接 durable execution，`Ledger` 保存 refs-only evidence，`Console` 消费 `current_owner_delta` 与 invocation-plan projection，`Foundry` 以内核编排 exact-version Agent 创建、接管和改进，`Connect` 把同一合同派生到 MCP / OpenAI / AI SDK / Skill / CLI / App 调用和分发面。`Console` 面向 operator 组织 current owner、next action、governance 和 drilldown；需要连接、安装、同步或工具调用时进入 `Connect` public entry；需要 durable attempt、queue、retry/dead-letter 或 runtime blocker 时进入 `Runway` public entry；需要 evidence、receipt ref、typed blocker ref 或 lineage 时进入 `Ledger` public entry。Agent Tool Arsenal / Capability Invocation OS 保持在适用品牌域的协同边界内，不新增独立品牌域。`Fabric` 由 Cloud authority 独立承载，不进入 Framework projection。ordinary App/CLI/operator route 只有 `current_owner_delta` 一条；Runway、Ledger、Atlas、Pack、Foundry Kernel、Connect 和 full drilldown 只能提供 safe action、refs、audit packet、FoundryRun/Owner action 或 support evidence。任何模块的 structural readiness、conformance pass、capability registry hit、tool result envelope、pack lock written、review receipt refs observed、provider completion、ledger verified、Console projection 或 App projection 都不能单独升级成 domain ready、quality/export ready、App release ready 或 production ready。
...
- `stage_progress_log` 是 OPL family-runtime attempt/progress projection：它从 OPL SQLite attempt ledger、Temporal provider status/history refs、human-gate / dead-letter state、domain-owned receipt / typed blocker refs 和 closeout refs 派生。Foundry Kernel 只消费该 projection 的 refs 作为 eval/improvement/read-model 输入，不拥有 runtime log，不写 provider history 或 attempt ledger，也不把 refs-only progress 写成 domain truth、quality verdict、artifact authority 或 runtime ownership。
- OPL Foundry Kernel 属于 Framework 内部 eval / improvement control plane：它把 descriptor、stage attempt、provider receipt、domain-owned eval/proof refs 和 operator blocker 组织成 lab run、improvement candidate、acceptance evidence 与 follow-up projection；它不接管 MAS/MAG/RCA 的 domain truth、quality verdict、artifact authority、memory body 或 owner receipt authority
- 在智能体自进化闭环中，OPL Foundry Kernel 负责 exact-version FoundryRun、冻结测试计划、EvidenceBundle、generation budget、risk-tiered Owner gate、canary / activation / rollback 和 App/operator projection；`opl-meta-agent` 只把 `DesignRequest` 或 `EvidenceBundle` 转成完整 `AgentBlueprint` / `EvalSpec` 或 `EvolutionProposal`；目标 domain owner 负责保护测试、最终 owner receipt、domain truth、quality verdict、权限授权和生产采用
...
- Runtime Manager / Runway / State Index / native helper 这组 provider-backed
  ...
  和 optional Node helper 加速；具体命令、字段、cache、SQLite sidecar
  和 helper lifecycle 的机器 truth 回到 runtime contracts、support docs、
  source/tests 和 live CLI。
- Foundry Kernel 与 family runtime supervision 的 refs-only control plane。它们只引用
...
Stage / route 的顶层运行语义固定为 `declared stage graph + decisive Codex route selection + route ABI guard + attempt ledger + passive transport persistence`。`Stage` 是 OPL 可启动、可恢复、可审计的主要开放语义判断，`StageRun` 是一次 durable 工单，`StageAttempt` 是同一工单内一次上下文隔离的 executor 调用；静态 pack 是否完整只影响 context 质量。`Route` 是终局 Codex Attempt 基于 domain artifact、diagnostic、owner recommendation 与 human input 做出的 advance、skip、repeat、reverse 或 route-back 选择。Transport reconciler 校验输出者是否有终局资格、字段是否规范、目标是否已声明，再把通过校验的选择物化成下一 StageRun request/projection 并对账 actual attempt/provider/currentness；它不能生成、批准、否决或自动替换 ABI 合法 route 的专业语义。Attempt ledger 只持久化 StageRun 内 Attempt 与外部 transport 观察，不构成第二控制面。
...
Cordis adoption 已完成默认 Framework Host composition cutover：DSH scoped `@deepseek-ai/cordis@4.0.1` 是 runtime dependency，`base-headless` 通过真实 profile 创建并销毁 composition，P3 inspect 回读 active default profile，P4 snapshot/digest contract 与 child composition refs 已启用，P5 vertical seams 和 P6 CLI/Runtime caller switch 已落地。家族品牌组合已经对齐 11 个当前 capability domains，其中 `Fabric` 由 Cloud authority 承载而不伪装成 Framework CLI module；源码进一步完成 13 个 source units / 6 个 target roots 的 physical topology cutover，以及两个 source-extracted workspace Package candidates，`src/modules/**` 已 retired/must-be-absent；Runway executor、Foundry evaluation、Connect discovery 已内联为 Framework Host-owned Cordis contribution。独立 Package publication、AionUI/Studio 的 release/runtime 准入、跨 GUI E2E 和各 owner live evidence 仍需 fresh readback。Cordis 不迁入 Temporal durable path、Package installed truth、Workspace bytes/binding、Ledger persistence、Foundry activation、domain truth、App product authority 或 Cloud control-plane truth；这些 owner-authoritative/live 事实继续 fresh readback。具体收益和运营门见 [`Cordis Adoption Plan`](./active/cordis-adoption-plan.md)。
