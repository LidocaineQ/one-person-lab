...
export type ExternalSandboxProviderAdapterConfig = {
  ...
  substrate: ExternalSandboxImplementedSubstrate | null;
  unsupportedSubstrate: string | null;
...
export function inspectExternalSandboxProviderAdapterEnv(
  ...
  const rawSubstrate = env.OPL_EXTERNAL_SANDBOX_SUBSTRATE?.trim().toLowerCase()
    || (env.OPL_CODEX_STAGE_SANDBOX_PROVIDER?.trim().toLowerCase() === 'e2b' ? 'e2b' : '');
  const substrate = EXTERNAL_SANDBOX_IMPLEMENTED_SUBSTRATES.includes(rawSubstrate as ExternalSandboxImplementedSubstrate)
    ...
    : null;
  const unsupportedSubstrate = rawSubstrate && substrate === null ? rawSubstrate : null;
  const missingRequiredEnv = [
    ...(rawSubstrate ? [] : ['OPL_EXTERNAL_SANDBOX_SUBSTRATE']),
  ...
  return {
    ...
    missingRequiredEnv,
    configured: substrate !== null && missingRequiredEnv.length === 0,
  };
}
...
export function buildExternalSandboxProviderAdapterPlan(targetRef: string) {
  ...
  const status = config.unsupportedSubstrate
    ? 'external_sandbox_provider_adapter_unsupported'
    : config.configured
