...
import { listOplConnections, readOplGatewayAccount } from '../../adapters/integration/public/app-state.ts';
  ...
  const sections = {
    ...
    agents: settingsSection('agents', 'Agents', 'agents', 'agent_packages_source_selection_and_developer_mode', [
      settingsItem({
        ...
        normal_summary: `Developer Mode is ${developerEffectiveState} in ${developerMode}; ${packageHealth} enabled Agent Packages have a fresh runnable readback.`,
...
function buildAppSettingsReadModel(
  ...
  return {
    ...
    codex_model_policy: {
      ...
      model_provider: asString(codex.model_provider) ?? asString(defaultProfile.model_provider),
      provider_name: asString(codex.provider_name) ?? asString(defaultProfile.provider_name),
...
function buildIssueQueue(input: BuildSettingsControlCenterInput): Array<Record<string, unknown>> {
  ...
  if (packageReadiness.attention_count > 0) {
    issues.push({
      ...
      label: 'Agent Package is not runnable',
      user_message: 'One or more installed and enabled Agent Packages do not have a fresh runnable readback.',
      severity: 'error',
