...
import { buildOplRuntimeEndpoints } from '../../../adapters/execution/index.ts';
import type { buildWorkspaceCatalog } from '../../../authority/workspace/index.ts';
...
export function buildCurrentReadinessProjection(
  ...
  const summary = {
    ...
    direct_entry_ready_projects_count: projects.filter((entry) => entry.binding_direct_entry_ready).length,
    manifest_ready_projects_count: projects.filter((entry) => entry.binding_manifest_ready).length,
    launch_ready_projects_count: projects.filter((entry) => entry.binding_launch_ready).length,
