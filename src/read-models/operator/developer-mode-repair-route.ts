...
import { loadStandardAgentEvaluationManifests } from '../../authority/packages/index.ts';
...
type DeveloperModeLiveCloseoutRoute = ReturnType<typeof buildDeveloperModeRepairRoute> & {
...
export type DeveloperModeRepairRouteInput = {
...
const DEVELOPER_MODE_REPAIR_AUTHORITY_BOUNDARY = {
  ...AUTHORITY_BOUNDARY,
  opl: 'console_developer_mode_patrol_repair_route_projection_refs_only',
...
const DEVELOPER_MODE_DYNAMIC_ROUTE_BUILDER = {
  surface_kind: 'opl_console_developer_mode_dynamic_repair_route',
  input_refs: [
...
function routeCloseoutRefs(receipt: DeveloperModeCloseoutReceipt) {
  return unique([
    ...
    ...receipt.foundry_activation_transaction_refs,
...
function developerModeScaleoutFollowthrough(verifiedReceipts: DeveloperModeCloseoutReceipt[]) {
  ...
  const foundryActivationTransactionRefs = unique(
    verifiedReceipts.flatMap((receipt) =>
      receiptRefListField(receipt, 'foundry_activation_transaction_refs')
    ),
  );
  ...
  const openGateIds = baseRoutesReady
    ? [
        ...
        foundryActivationTransactionRefs.length > 0
          ? null
          : 'foundry_activation_transaction_refs',
  ...
  return {
    ...
    foundry_activation_transaction_ref_count: foundryActivationTransactionRefs.length,
    ...
    required_return_shapes: [
      ...
      'foundry_activation_transaction_ref',
    ...
    payload_ref_hints: {
      ...
      foundry_activation_transaction_refs_should_cover:
        'immutable Foundry activation transaction refs bound to an exact AgentVersion and Owner authority receipt',
...
export function buildDeveloperModeRepairRoute(input: DeveloperModeRepairRouteInput) {
  ...
  return {
    surface_kind: 'opl_console_developer_mode_dynamic_repair_route',
    version: 'opl-console.v1.developer-mode-dynamic-repair-route',
...
export function buildDeveloperModeRepairRouteReadModel(input: {
  ...
  const manifestCloseoutDrills: DeveloperModeLiveCloseoutRoute[] = evaluationManifests
    .flatMap((manifest) => manifest.closeout_drills.map((drill) => ({
      ...buildDeveloperModeRepairRoute(
        drill as unknown as DeveloperModeRepairRouteInput,
      ),
  ...
  const liveCloseoutEvidence = {
    surface_kind: 'opl_console_developer_mode_live_closeout_evidence_read_model',
    version: 'opl-console.v1.developer-mode-live-closeout-evidence',
    ...
    refs_only: true,
    evidence_scope: 'developer_mode_repair_closeout_drills_and_verified_live_ledger_receipts',
    ...
    foundry_activation_transaction_receipt_refs: verifiedLedgerReceipts.flatMap((receipt) =>
      receiptRefListField(receipt, 'foundry_activation_transaction_refs')
    ),
    ...
    summary: {
      ...
      foundry_activation_transaction_ref_count:
        scaleoutFollowthrough.foundry_activation_transaction_ref_count,
  ...
  return {
    surface_kind: 'opl_console_developer_mode_repair_route_read_model',
    version: 'opl-console.v1.developer-mode-repair-route',
    ...
    inputs: {
      ...
      patrol_observation_ref: 'developer-mode-patrol-observation-ref',
    },
    route_policy: {
    ...
    patrol_projection: {
      patrol_ref: 'developer-mode-patrol-ref:default',
