...
- 当前生态按 `OPL Base + OPL App + OPL Packages + OPL Cloud` 阅读：`OPL Framework` 实现 Base，并持有 `framework_runtime_package_graph_and_app_projection` scope 内唯一的 Cordis Host；`one-person-lab-app` 持有唯一 App 产品、GUI ABI、Client profile 与发布 authority，`opl-aion-shell` 是当前 Stable Shell，`opl-studio` 是完整的 DSH/Cordis Application Host 候选实现，其 scope 仅为 `dsh_profile_plugin_lifecycle_codex_and_delivery_transport_composition`。两个 Host 的并存不转移 OPL runtime、Package registry/currentness、App state/action、domain、product 或 release authority，机器边界见 `contracts/opl-framework/cordis-architecture-profile.json#host_scope_boundary`。Packages 是可独立安装、版本化和发布的能力产品，MAS/MAG/RCA 等 Foundry Agents 是其中保留领域 authority 的专业家族；Cloud 是正在落地的在线产品层。品牌模块/认知域、authority owner、Package 发布单元和 Cordis plugin contribution 是四种独立结构，不应机械一一对应
- 当前公开默认主路径是 `One Person Lab App or CLI -> Codex CLI first-class executor -> OPL activation / stage-attempt projection layer -> Temporal-backed family runtime provider -> selected installed Package entry`。普通 App path 通过动态 installed descriptor 与 Framework/App projection 呈现已安装的 Foundry Agent 或其他 Package 能力，不维护内置 Package/Agent 清单；AionUI backend / Agent selector 与非默认 executor adapter 只属于显式 developer/operator diagnostic 或 stage binding。Full OPL family readiness 的 online runtime substrate 是已配置且 ready 的 Temporal provider，Hermes-Agent 在 provider/readiness/compat 语境只作为历史 provenance、参考材料、诊断语料或负向 guard，`hermes_agent`、`claude_code` 与 `antigravity_cli` 同属 canonical 显式非默认 executor adapter/backend，只承诺接口、生命周期、receipt、audit 与 fail-closed gate，不承诺质量、工具语义或 resume 与 `Codex CLI` 等价；`local_sqlite` 只作为 retired-provider negative guard，SQLite sidecar 只作为 projection/readback index
...
普通 operator / App 默认读取 `opl framework readiness --family-defaults --json`，Stage 默认聚合读取 `opl stages readiness --family-defaults --json`，详细诊断按需展开。Stage 内策略由 selected executor 执行；domain verdict 与 App release truth 分别由 domain owner 和 App owner 提供。`contracts/opl-framework/public-surface-index.json` 列出 active surfaces、owners、routes 和 notes。
...
- `contracts/opl-framework/brand-module-surfaces.json`：当前 OPL 品牌模块的自身 executable surface 合同；`opl charter|atlas|workspace|pack|stagecraft|runway|ledger|console|foundry|connect status|inspect|interfaces|validate|doctor --json` 从这里派生，证明模块级 Workspace-level `L4_structural_baseline`，但不声明 domain ready、quality verdict、artifact authority、production ready、owner receipt、typed blocker 或 App release truth。
...
- `contracts/opl-framework/target-operating-architecture-contract.json`：OPL family 顶层目标操作架构合同；它冻结标准资源模型、Codex CLI 单一 stage 语义路由 owner、Domain Pack + generated surfaces + authority ABI、passive transport/currentness projection、Atlas/Ledger telemetry、App Console 默认字段与 Foundry Kernel refs-only improvement 边界。
...
- `contracts/opl-framework/public-surface-index.json`：active public surface 索引，列出每个 surface 的 owner、refs、routes 和 notes
- `contracts/opl-framework/README.md`：这些 active JSON contract 的人类可读说明
- `contracts/opl-framework/runtime-manager-contract.json`：当前 OPL Runtime Manager 产品控制面合同；它冻结 OPL 如何管理 provider-backed family runtime、StageRun durable invocation/spec、pre-start launch registry、stage-attempt request/projection、domain dispatch、可选 native helper lifecycle、高频状态索引与 freshness 口径，同时明确不复制 domain truth 或 quality authority
...
- `contracts/opl-framework/stage-route-transport-contract.json`：OPL Stage/route 顶层调度合同；它固定 Stage graph、StageRun、Attempt、owner-route 输入、ABI guard、durable target launch 与 attempt ledger 的分层，明确权威跨 Stage route 只能由 progress-terminal decisive Codex Attempt 输出，controller 只校验并物化目标 StageRun
- `contracts/opl-framework/cognitive-computation-kernel.json`：OPL 认知计算内核合同；它固定 Stage 内 candidate generation、reflection / review、comparative selection、evolution / revision、strategy retrospective、tool affordance boundary、knowledge 和独立 quality gate 的 refs-only 组织边界，同时明确 strategy retrospective 不是跨 Stage Meta Review、工具目录不是流程脚本、Route 不是小 Stage、OPL 不持有 domain truth 或质量裁决
- `contracts/opl-framework/state-index-kernel-contract.json`：OPL State Index Kernel 合同；它冻结 File Truth + durable pre-start StageRun transport registry + SQLite Sidecar Index + Temporal Runtime 分工。`stage_run_launches` 只保存 bounded exact launch envelope 与 transport receipt 以恢复启动窗口，其余 SQLite 面保持可重建索引；两者都不存 domain truth、memory/artifact body、owner receipt authority、quality/export verdict 或 production readiness
...
- `contracts/opl-framework/native-helper-contract.json`：OPL Node standard-library helper 的 JSON stdio 合同；它冻结 `opl-sysprobe`、`opl-doctor-native`、`opl-runtime-watch`、`opl-artifact-indexer` 与 `opl-state-indexer` 的输入输出边界，以及 helper 的 doctor / repair / package source 面。`owner_split.domain_truth_owners` 由 `src/kernel/standard-agent-registry.ts` 的 `standard_domain_agent` 成员投影，ScholarSkills 等 capability package 不进入该列表
