...
- 当前生态按 `OPL Base + OPL App + OPL Packages + optional OPL Cloud` 阅读：`OPL Framework` 实现 Base 并作为唯一 Cordis Host；`one-person-lab-app` 持有唯一 App 产品、GUI ABI、Client profile 与发布 authority，`opl-aion-shell` 是当前 Stable Shell，`opl-studio` 是 DSH-derived 候选 Shell；Packages 是可独立安装、版本化和发布的能力产品，MAS/MAG/RCA 等 Foundry Agents 是其中保留领域 authority 的专业家族；Cloud 是可选在线产品层，不是第二 Host。品牌模块/认知域、authority owner、Package 发布单元和 Cordis plugin contribution 是四种独立结构，不应机械一一对应
