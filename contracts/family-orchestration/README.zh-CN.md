...
3. Controlled StageRun canary evidence 是标准 domain adoption 证据面；它固定 candidate generation、grounded reflection、comparative selection、evolution / revision、strategy retrospective 与 independent quality gate 的 refs-only role artifact shape，并由 `opl agents conformance` 验证。该 evidence scope 必须是 `controlled_fixture_not_live_domain_progress`，不能声明 live paper / grant / visual / target-agent progress、quality verdict、artifact ready、export ready 或 production ready。
...
`ProgressCloseoutProjection` 是 OPL Framework / Runway 的 closeout 投影。selected executor 可以返回 typed JSON，也可以返回普通可读输出。OPL 持久化 raw output，并自动派生 refs、hash、lineage 和最小 progress envelope；解析、schema、receipt 或格式缺口只形成质量债务，不得阻断下一 stage。只有在没有任何可读 artifact 时，provider / infrastructure failure 才能阻断。OPL 仍不得伪造 typed blocker、owner receipt、human gate、domain truth、artifact body、memory body、quality verdict、runtime ready、domain ready 或 production ready。
...
admitted domain 的 action catalog 可以内嵌 `family_action_catalog`，也可以声明仓内固定 locator `{ "ref_kind": "repo_path", "ref": "contracts/action_catalog.json" }`；两者同时出现会 fail closed。stage control plane 不接受 inline body，只接受 `{ "ref_kind": "generated_surface", "ref": "opl-generated:family_stage_control_plane", "source_ref": "agent/stages/manifest.json" }`；Atlas 调用 OPL Pack 的 canonical compiler 从 domain repo 的 declarative manifest 生成并经 Stagecraft normalize 后物化到 read model，不读取或要求 repo-tracked `contracts/stage_control_plane.json`。两种 ref 都采用 exact field allowlist；错误 kind/ref/source、缺少 source manifest、URL、备用路径或额外 fallback 字段都会 fail closed。
...
旧 `family_transition_spec`、matrix case、visual adapter 与 domain oracle 只按 inert diagnostic input 兼容读取，不再物化、执行、校验 guard 或参与 admission。Codex CLI 是唯一语义 route owner；`family_action_catalog` 与 `family_stage_control_plane` 只声明 action、stage 范围和可观测元数据，OPL 负责 transport、attempt lifecycle、identity/currentness 与 declared-stage scope，不生成或否决语义路线。
...
`family-stage-cohort-loop` 是 refs-only diagnostic lens，覆盖 source scope、cohort query、trigger 和 monitor/metric refs。缺少环节会给出 finding 和 minimal counterexample，但不进入 runtime launch path，也不阻止 Codex 推进。这个投影只服务 App/operator 的质量与 ready drilldown。
...
Stage graph、StageRun、Attempt 与 owner-route 输入的关系由 `contracts/opl-framework/stage-route-transport-contract.json` 冻结：Stage 表达一个主要开放语义判断，StageRun 是一次 durable 工单，Attempt 是同一 StageRun 内一次上下文隔离的 executor 调用。`family-owner-route` 承载 domain owner 的下一步/route-back 建议以及 typed blocker、safe action、owner receipt 或 handoff refs；只有 progress-terminal decisive Codex Attempt 将其写成 `route_impact.stage_route_decision` 后，它才成为权威跨 Stage 路由。OPL 只校验 ABI 与 Attempt authority并运输 declared target，不做专业语义判断。route 不是 graph 里的隐藏小 Stage，也不能绕过 Attempt ledger 执行。
...
`opl_stage_launch_invocation` 是被动 transport event：默认 `codex_cli` 可从 requested/declared stage、hydrated prompt、workspace 与任意可读 prior artifact 启动。`bounded_edit_ref`、stage packet、manifest、receipt 与 capability binding 都是可选质量上下文，缺失不阻止 stage。非默认 executor 仍必须有显式 adapter/binding，因为这是实际可执行边界；该要求不构成第二套语义路由控制面。
