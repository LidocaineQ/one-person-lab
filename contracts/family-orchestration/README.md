...
Readiness is the default operator/App aggregation surface and detailed diagnostics are requested explicitly. The selected executor runs stage-internal strategy from the admitted Stage pack. Framework owns activation, runtime transport, and projections; domain owners provide domain truth and verdicts; the App repository owns GUI and release truth. `public-surface-index.json` lists the active surfaces and their routes.
...
A stage descriptor may carry `display_names` as a locale-to-label map. Legacy family planes may omit it; when present, locale keys and values must be non-empty, `en-US` is required, and `display_names.en-US` must exactly equal the compatibility `title`. Canonical normalization preserves every declared locale entry and never invents translations.
