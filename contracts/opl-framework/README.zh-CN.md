...
- stage-led 任务选择
- `OPL Base`、`OPL App`、`OPL Packages` 与可选 `OPL Cloud` 的生态所有权
...
- brand-module governance 通过 registry、模块自有品牌 CLI command surface、read-model projection、validation gate 和 false-authority boundary，让 Charter、Atlas、Workspace、Pack、Stagecraft、Runway、Ledger、Console、Foundry Kernel 与 Connect 保持 Workspace-level structural baseline；品牌域只作跨 Framework/App/Cloud 认知地图，不定义物理 source owner、Package 或 plugin 数量
...
- target operating architecture 治理，把理想资源模型、Codex-owned 语义路由、Declarative Domain Pack / authority ABI、被动 transport/currentness、Atlas/Ledger telemetry、App Console 收薄与 Foundry Kernel 生命周期边界冻结成顶层机器合同
...
- cognitive computation kernel 治理，让每个 Stage 声明 generation、reflection、comparative selection、evolution、strategy retrospective、tool affordance boundary、knowledge 与独立 quality gate refs；工具目录是 affordance catalog，不是 workflow script；owner-route 只是领域输入/建议，权威跨 Stage Route 由终局 decisive Codex Attempt 判断，且 Route 不是小 Stage
...
当前生态模型是 `OPL Base + OPL App + OPL Packages + optional OPL Cloud`。`one-person-lab` 实现 Base 并持有唯一 Framework Cordis Host；`one-person-lab-app` 持有唯一 App 产品、GUI ABI、Client profile、发布组合和 active-shell 决策；`opl-aion-shell` 是当前 Stable Shell，`opl-studio` 是 DSH-derived 候选 Shell。Packages 是可独立安装、版本化和发布的能力产品，MAS/MAG/RCA 作为 Foundry Agent Package 家族成员保留各自领域 authority；Cloud 是可选在线产品层，不是第二 Cordis Host。品牌/认知域、authority owner、Package 发布单元与 Cordis plugin contribution 是四种独立结构。执行链路仍是 `Codex CLI first-class executor + explicit OPL activation + configured family runtime provider + installed Package discovery`。

## 当前真相应去哪里看

当前 `OPL Base / App / Packages / optional Cloud` 生态模型应优先回到：
